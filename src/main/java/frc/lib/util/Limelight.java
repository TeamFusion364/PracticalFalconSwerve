...
public class Limelight {
  ...
  /**
   * @return Horizontal Offset From Crosshair To Target in degrees (Note: Inverted from standard LL
   *     provided angle to be CCW+)
   */
  ...
  public boolean hasRetroTarget() {
    boolean hasTarget =
        NetworkTableInstance.getDefault().getTable(LLNTname).getEntry("tv").getDouble(0) == 1;
    return hasTarget;
  }
  ...
  public double getLatency() {
    double pipelineLatency =
        NetworkTableInstance.getDefault().getTable(LLNTname).getEntry("tl").getDouble(0) / 1000;
    double captureLatency =
        NetworkTableInstance.getDefault().getTable(LLNTname).getEntry("cl").getDouble(0) / 1000;
